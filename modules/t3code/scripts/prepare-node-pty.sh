...
if [[ ! -f "$bundled_pty" ]]; then
  echo "[t3code] Missing bundled node-pty module: $bundled_pty" >&2
  exit 1
fi

if [[ ! -d "$runtime_versions" ]]; then
  echo "[t3code] No downloaded T3 runtime versions at $runtime_versions; skipping node-pty repair"
  exit 0
fi
...
while IFS= read -r -d '' package_json; do
  ...
  if [[ ! -f "$target" ]]; then
    ...
    echo "[t3code] Restored node-pty native module for ${node_pty_dir#"$runtime_versions"/}"
  fi
